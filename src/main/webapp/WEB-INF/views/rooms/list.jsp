...
<div class="table-responsive">
	<table class="table table-hover">
		<tr>
			...
			<th class="v-alighn"><spring:message
					code="lbl.room.roomEquipments" /></th>
			...
			<th class="text-center v-alighn"><a
				href="${RoomController.ROOM_CREATE_URL}"
				title="<spring:message code="lbl.room.createRoom"/>"> <i
		...
		<form:form role="form" action="rooms" method="get"
			...
			<td>
				<div class="form-group">
					<spring:message code="lbl.room.filterMinCapacity"
						var="filterMinCapacity" />
					<form:input type="number" class="input-number" path="minCapacity"
						step="1" required="true" title="${filterMinCapacity}" />
					<spring:message code="lbl.room.filterMaxCapacity"
						var="filterMaxCapacity" />
					<form:input type="number" class="input-number" path="maxCapacity"
						step="1" required="true" title="${filterMaxCapacity}" />
				</div>
			</td>
			<td>
				<div class="form-group">
					<form:select class="form-control" path="equipments" id="equipments"
						...
						<c:forEach items="${equipments}" var="equipment">
							...
							<c:forEach items="${roomFilter.equipments}"
								var="equipmentInFilter">
								<c:if test="${!found}">
		...
		<c:forEach items="${rooms}" var="room">
			<tr>
				...
				<td class="text-center v-alighn"><a
					...
					title="<spring:message code="lbl.room.editRoom"/>"> <i
