...
public class UserGroupFilterSpecification implements Specification<UserGroup> {
    ...
    private void findByName() {
        if (userGroupFilter.getName() != null
                && !userGroupFilter.getName().trim().equals("")) {
            list.add((root, criteriaQuery, criteriaBuilder) -> criteriaBuilder
                    .like(root.get(UserGroup_.name),
    ...
    private void findByCuratorId() {
        if (userGroupFilter.getCuratorId() != null
                && userGroupFilter.getCuratorId() >= 0) {
            list.add((root, criteriaQuery, criteriaBuilder) -> root
                    .get(UserGroup_.curator)
    ...
    private void findByLevelId() {
        if (userGroupFilter.getLevelId() != null
                && userGroupFilter.getLevelId() >= 0) {
            list.add((root, criteriaQuery, criteriaBuilder) -> root
                    .get(UserGroup_.level).in(userGroupFilter.getLevelId()));
        }
    }
    ...
    private void setSortingParameters(final Root<UserGroup> root,
        ...
        if (userGroupFilter.getSortOrder() == Order.ASC.ordinal()
                && userGroupFilter.getFieldForSorting() == 1) {
            criteriaQuery.orderBy(
                    criteriaBuilder.asc(root.get(UserGroup_.name.getName())));
        }
        if (userGroupFilter.getSortOrder() == Order.DESC.ordinal()
                && userGroupFilter.getFieldForSorting() == 1) {
            criteriaQuery.orderBy(
                    criteriaBuilder.desc(root.get(UserGroup_.name.getName())));
        }
        if (userGroupFilter.getSortOrder() == Order.ASC.ordinal()
                && userGroupFilter.getFieldForSorting() == 3) {
            criteriaQuery.orderBy(
                    criteriaBuilder.asc(root.get(UserGroup_.level.getName())));
        }
        if (userGroupFilter.getSortOrder() == Order.DESC.ordinal()
                && userGroupFilter.getFieldForSorting() == 3) {
            criteriaQuery.orderBy(
                    criteriaBuilder.desc(root.get(UserGroup_.level.getName())));
        }
        if (userGroupFilter.getSortOrder() == Order.ASC.ordinal()
                && userGroupFilter.getFieldForSorting() == 4) {
            criteriaQuery.orderBy(criteriaBuilder
                    .asc(criteriaBuilder.size(root.<Collection>get("users"))));
        }
        if (userGroupFilter.getSortOrder() == Order.DESC.ordinal()
                && userGroupFilter.getFieldForSorting() == 4) {
            criteriaQuery.orderBy(criteriaBuilder
                    .desc(criteriaBuilder.size(root.<Collection>get("users"))));
        }
