...
public class UserGroupValidator
        implements ConstraintValidator<Validate, UserGroupDTO> {
     ...
     */
    @Override
    public boolean isValid(final UserGroupDTO userGroupDTO,
            ConstraintValidatorContext context) {
        isCorrectLengthName = hasCorrectNameLength(userGroupDTO.getName());
        isUniqueName = hasUniqueName(userGroupDTO.getId(),
                userGroupDTO.getName());
        isNotEmptyDescription = hasDescription(userGroupDTO.getDescription());

        showErrors(isUniqueName, isCorrectLengthName, isNotEmptyDescription,
                context);
    ...
    public boolean hasCorrectNameLength(final String groupName) {
        if (groupName.length() < minNameLength
     ...
     */
    private void showErrors(boolean isUniqueName, boolean isCorrectLengthName,
            boolean isNotEmptyDescription, ConstraintValidatorContext context) {
        if (!isCorrectLengthName) {
            errorMessage(ValidationFields.NAME,
                    ValidationMessages.INVALID_GROUP_NAME_LENGTH, context);
        }
        if (!isUniqueName) {
            errorMessage(ValidationFields.NAME,
                    ValidationMessages.INVALID_GROUP_NAME_UNIQUENESS, context);
        }
        if (!isNotEmptyDescription) {
            errorMessage(ValidationFields.DESCRIPTION,
                    ValidationMessages.INVALID_GROUP_DESCRIPTION, context);
        }
    }
}
