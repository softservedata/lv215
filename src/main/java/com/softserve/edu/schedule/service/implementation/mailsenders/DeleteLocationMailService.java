...
public class DeleteLocationMailService implements MailConstants {
	 ...
	 */
	@Async
	public void sendInfoMessageLocationDelete(final User user, final Location location, final Locale locale) {
		...
		try {
			...
			message.setSubject(messageSource.getMessage(LOCATION_DELETE_MESSAGE, new String[0], locale));
			String htmlContent = this.templateEngine.process(LOCATION_DELETE_TEMPLATE, ctx);
