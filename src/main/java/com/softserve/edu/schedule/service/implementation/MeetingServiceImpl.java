...
public class MeetingServiceImpl implements MeetingService {
	...
	public void update(final MeetingDTO meetingDTO) {
		if (meetingDTO.getStatus() == MeetingStatus.FINISHED || meetingDTO.getStatus() == MeetingStatus.ARCHIVED) {
			return;
		}
	...
	public void sendMailIfStatusChanged(final MeetingDTO meetingDTO) {
		...
		if ((meetingDao.getById(meetingDTO.getId()).getStatus() == MeetingStatus.APPROVED)
				&& ((meetingDTO.getStatus() == MeetingStatus.DISAPPROVED)
						|| (meetingDTO.getStatus() == MeetingStatus.NOT_APPROVED))) {
			meetingChangeStatusMailService.sendInfoMessageAboutMeetingStatusChanging(
					meetingDao.getById(meetingDTO.getId()), LocaleContextHolder.getLocale());
		}
	 ...
	 */
	@Override
	public List<MeetingForCalendarDTO> getMeetingsInIntervalByGroupId(final Long groupId, final String start,
			final String end) {
	 ...
	 */
	@Override
	public List<MeetingForCalendarDTO> getMeetingsInIntervalByRoomId(final Long roomId, final String start,
			final String end) {
		LocalDate startDate = LocalDate.parse(start);
		LocalDate endDate = LocalDate.parse(end);
	 ...
	 */
	@Override
	public List<MeetingForCalendarDTO> getMeetingsInIntervalBySubjectId(final Long subjectId, final String start,
			final String end) {
		LocalDate startDate = LocalDate.parse(start);
		LocalDate endDate = LocalDate.parse(end);
	 ...
	 */
	@Override
	public List<MeetingForCalendarDTO> getMeetingsInIntervalByUserId(final String userId, final String start,
			final String end) {
		LocalDate startDate = LocalDate.parse(start);
		LocalDate endDate = LocalDate.parse(end);
	...
	public Map<String, Long> getMeetingsForChartInIntervalByUserId(String userId, String start, String end) {
		...
		LocalDate endDate = LocalDate.parse(end);
		Long id = Long.parseLong(userId);
		return meetingForChartDTOConverter
				.getChartData(meetingDao.getMeetingsInIntervalByUserId(id, startDate, endDate));
	}
}
