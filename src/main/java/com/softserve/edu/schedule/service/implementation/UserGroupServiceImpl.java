...
import com.softserve.edu.schedule.service.implementation.dtoconverter.UserGroupForAndroidDTOConverter;
...
public class UserGroupServiceImpl implements UserGroupService {
     ...
     */
    @Autowired
    private UserGroupForAndroidDTOConverter userGroupForAndroidDTOConverter;
    ...
    public List<UserGroupDTO> getAll() {
        return userGroupDAO.getAll().stream()
                .map(s -> userGroupDTOConverter.getDTO(s))
     ...
     */
    @Override
    public UserGroupDTO addUserToGroup(final UserDTO userDTO,
     ...
     */
    @Override
    public List<UserGroupDTO> getUserGroupPageWithFilter(
            UserGroupFilter userGroupFilter, Paginator userGroupPaginator) {
        return userGroupDAO
                .getUserGroupPageWithFilter(userGroupFilter, userGroupPaginator)
                .stream().map(e -> userGroupDTOConverter.getDTO(e))
                .collect(Collectors.toList());
    }

    @Override
    public List<UserGroupDTO> getUserGroupsByName(final String userGroupName) {
        return userGroupDAO.getGroupsByName(userGroupName).stream()
                .map(s -> userGroupDTOConverter.getDTO(s))
                .collect(Collectors.toList());
    }

    /**
     * Returns a List of UserGroup objects for android devices.
     *
     * @return List of UserGroupForAndroidDTO objects
     */
    @Override
    public List<UserGroupForAndroidDTO> getAllForAndroid() {
        return userGroupDAO.getAll().stream()
                .map(s -> userGroupForAndroidDTOConverter.getDTO(s))
                .collect(Collectors.toList());
    }
}
