 ...
 * @since 1.8
 */
public class MeetingValidator implements ConstraintValidator<Validate, MeetingDTO> {
	 ...
	 */
	@Override
	public boolean isValid(final MeetingDTO meetingDTO, ConstraintValidatorContext context) {
		...
		printErrorMessages(isValidDescription, isValidDate, isValidStartTime, isValidEndTime,
				isValidEndTimeToCompareStartTime, isValidMultiselectGroups, isValidLevel, isValidMeetingStatus, isOriginMeeting, context);
		return (isValidDescription && isValidDate && isValidStartTime && isValidEndTime
				&& isValidEndTimeToCompareStartTime && isValidMultiselectGroups && isValidLevel && isValidMeetingStatus && isOriginMeeting);
	}
	...
	private boolean isValidDate(MeetingDTO meetingDTO) {
		return meetingDTO.getDate() != null && meetingDTO.getDate().isAfter(LocalDate.now().minusDays(1));
	}
	...
	private void printErrorMessages(boolean isValidDescription, boolean isValidDate, boolean isValidStartTime,
			boolean isValidEndTime, boolean isValidEndTimeToCompareStartTime, boolean isValidMultiselectGroups,
			boolean isValidLevel,boolean isValidMeetingStatus, boolean isOriginMeeting, ConstraintValidatorContext context) {
		if (!isValidDescription) {
			errorMessage(ValidationFields.DESCRIPTION, ValidationMessages.INVALID_CHARACTERS, context);
		}
		if (!isValidDate) {
			errorMessage(ValidationFields.DATE, ValidationMessages.INVALID_DATE, context);
		}
		if (!isValidStartTime) {
			errorMessage(ValidationFields.STARTTIME, ValidationMessages.INVALID_TIME, context);
		}
		if (!isValidEndTime) {
			errorMessage(ValidationFields.ENDTIME, ValidationMessages.INVALID_TIME, context);
		}
		if (!isValidEndTimeToCompareStartTime) {
			errorMessage(ValidationFields.ENDTIME, ValidationMessages.INVALID_ENDTIME_COMPARE_STARTTIME, context);
		}
		if (!isValidMultiselectGroups) {
			errorMessage(ValidationFields.GROUPS, ValidationMessages.EMPTY_FIELD, context);
		}
		if (!isValidLevel) {
			errorMessage(ValidationFields.LEVEL, ValidationMessages.INVALID_LEVEL, context);
		}
		if (!isValidMeetingStatus) {
			errorMessage(ValidationFields.STATUS, ValidationMessages.INVALID_STATUS, context);
		}

		if (!isOriginMeeting) {
			errorMessage(ValidationFields.SUBJECT, ValidationMessages.DUPLICATE_MEETING, context);
		}
	}
}
