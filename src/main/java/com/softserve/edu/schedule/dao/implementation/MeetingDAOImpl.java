...
public class MeetingDAOImpl extends CrudDAOImpl<Meeting> implements MeetingDAO {
	...
	public List<Meeting> getUnfinishedPastMeetings() {
		...
		todayPredicate = builder.and(todayPredicate, builder.equal(root.get(Meeting_.date), LocalDate.now()));
		todayPredicate = builder.and(todayPredicate, builder.lessThan(root.get(Meeting_.endTime), LocalTime.now()));

		Predicate timePredicate = builder.disjunction();
		timePredicate = builder.or(timePredicate, builder.lessThan(root.get(Meeting_.date), LocalDate.now()));
		timePredicate = builder.or(timePredicate, todayPredicate);

		Predicate searchPredicate = builder.conjunction();
		searchPredicate = builder.and(searchPredicate,
				builder.not(root.get(Meeting_.status).in(MeetingStatus.FINISHED, MeetingStatus.ARCHIVED)));
	 ...
	 */
	@Override
	public List<Meeting> getMeetingsInIntervalByRoomId(final Long roomId, final LocalDate startDate,
	 ...
	 */
	@Override
	public List<Meeting> getMeetingsInIntervalBySubjectId(final Long subjectId, final LocalDate startDate,
			final LocalDate endDate) {
		CriteriaBuilder builder = getEm().getCriteriaBuilder();
		CriteriaQuery<Meeting> cq = builder.createQuery(Meeting.class);
		Root<Meeting> root = cq.from(Meeting.class);
	 ...
	 */
	@Override
	public List<Meeting> getMeetingsInIntervalByUserId(final Long userId, final LocalDate startDate,
			final LocalDate endDate) {
		CriteriaBuilder builder = getEm().getCriteriaBuilder();
		CriteriaQuery<Meeting> cq = builder.createQuery(Meeting.class);
		Root<Meeting> root = cq.from(Meeting.class);
	 ...
	 */
	@Override
	public List<Meeting> getMeetingsInIntervalByGroupId(final Long groupId, final LocalDate startDate,
			final LocalDate endDate) {
		CriteriaBuilder builder = getEm().getCriteriaBuilder();
		CriteriaQuery<Meeting> cq = builder.createQuery(Meeting.class);
		Root<Meeting> root = cq.from(Meeting.class);
	...
	/**
	 * Method returns all meetings in the DB which date and time are in past and
	 * status isn't ARCHIVED.
	 *
	...
	public List<Meeting> getPastNotArchivedMeetings() {
		...
		Root<Meeting> root = cq.from(Meeting.class);
		Predicate predicate = builder.conjunction();
		predicate = builder.and(predicate, builder.lessThan(root.get(Meeting_.date), LocalDate.now()));
		predicate = builder.and(predicate, builder.not(root.get(Meeting_.status).in(MeetingStatus.ARCHIVED)));
		cq.where(predicate);
		cq.distinct(true);
		return getEm().createQuery(cq).getResultList();
	}
